...
//alert(domMenu_userAgent);
var domMenu_isOpera = domMenu_userAgent.indexOf('opera 7') != -1 ? 1 : 0;
var domMenu_isKonq = domMenu_userAgent.indexOf('konq') != -1 ? 1 : 0;
var domMenu_isIE = !domMenu_isKonq && !domMenu_isOpera && domMenu_userAgent.indexOf('msie 5.0') == -1 && document.all ? 1 : 0;
var domMenu_isIE5 = domMenu_isIE && domMenu_userAgent.indexOf('msie 5.') != -1;
var domMenu_isGecko = domMenu_userAgent.indexOf('gecko') != -1 ? 1 : 0;
//var domMenu_isGecko = domMenu_userAgent.indexOf('mozilla') != -1 ? 1 : 0;
...
function domMenu_activate(in_containerId)
{
    ...
    for (var index = 1; index <= numSiblings; index++) {
        ...
        rootMenuTableCell.data.merge(new domMenu_Hash(
            ...
            'numChildren', rootMenuTableCell.data.indexLength,
            'childElements', new domMenu_Hash(),
        ...
        rootElement.innerHTML = rootMenuTableCell.data.items['contents'];
...
function domMenu_activateSubMenu(in_parentElement)
{
    ...
    for (var index = 1; index <= numSiblings; index++) {
        ...
        menuTableCell.data.merge(new domMenu_Hash(
            ...
            'numChildren', menuTableCell.data.indexLength,
            'childElements', new domMenu_Hash(),
            'offsets', new domMenu_Hash(),
...
function domMenu_changeActivePath(in_newActiveElement, in_oldActiveElement, in_closeDelay)
{
    ...
    if (in_newActiveElement) {
        ...
        if (in_oldActiveElement == in_newActiveElement.data.items['parentElement']) {
            for (var i in newActivePath.items) {
                domMenu_cancelTimeout(i, 'close'); 
                domMenu_toggleHighlight(newActivePath.items[i], true);
            }

            return in_newActiveElement;
        }
        // if the new active element is in the old active path, then preserve old active path
        // and the old active element
        else if (oldActivePath.hasItem(in_newActiveElement.id)) {
            for (var i in oldActivePath.items) {
                domMenu_cancelTimeout(i, 'close');
                if (in_oldActiveElement != oldActivePath.items[i] || in_oldActiveElement == in_newActiveElement) {
                    domMenu_toggleHighlight(oldActivePath.items[i], true);
                }
            }

            return in_oldActiveElement;
        }
    ...
    for (var i in oldActivePath.items) {
        ...
        if (isRootLevel) {
            ...
            var tmp_function = 'domMenu_toggleSubMenu(argv[0], ' + domMenu_quote('hidden') + '); domMenu_toggleHighlight(argv[0], false);';
...
function domMenu_closeEvent(in_this, in_event)
{
    ...
    // we are highlighting the top level, but menu is not yet 'active'
        ...
        if (currentTarget != relatedTarget) {
            domMenu_changeActivePath(false, currentTarget);
        }
    }
}    
...
function domMenu_toggleHighlight(in_element, in_status)
{
    ...
    in_element.firstChild.className = className;

    // force konqueror to change the styles
    if (domMenu_isKonq) {
        in_element.firstChild.style.display = 'none';
        in_element.firstChild.style.display = '';
    }
}
...
function domMenu_resolveLink(in_this, in_event)
{
    ...
    if (currentTarget.data.items['uri']) {
        if (!currentTarget.data.items['target']) {
            currentTarget.data.setItem('target', '_self');
        }
