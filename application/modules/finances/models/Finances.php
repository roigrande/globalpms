...
/**
 * This is the Data Mapper class for the Acl_financess table.
 */
class Finances_Model_Finances {
    ...
    public function getTable() {
        if (null === $this->_table) {
            $this->_table = new Finances_Model_DbTable_Finances();
        }
        return $this->_table;
    }
    ...
    public function fetchSql() {
        $sql = "SELECT financess.id, financess.name, date,
                    email,status, roles.name as role
          FROM financess, roles
          WHERE financess.role_id = roles.id              
          ORDER BY roles.id";

        $table = $this->getTable()->getAdapter()->fetchAll($sql);
        //   Zend_Debug::dump($table,"Finances");
