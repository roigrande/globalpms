...
/**
 * This is the Data Mapper class for the Acl_productionssuppliers table.
 */
class Supplier_Model_Productionssupplier {
    ...
    public function getTable() {
        if (null === $this->_table) {
            $this->_table = new Supplier_Model_DbTable_Productionssupplier();
        }
        return $this->_table;
    }
...
//        die();
    ...
    public function fetchSql() {
        $sql = "SELECT productionssuppliers.id, productionssuppliers.name, date,
                    email,status, roles.name as role
          FROM productionssuppliers, roles
          WHERE productionssuppliers.role_id = roles.id              
          ORDER BY roles.id";

        $table = $this->getTable()->getAdapter()->fetchAll($sql);
        //   Zend_Debug::dump($table,"Productionssupplier");
     ...
     * @return array
     */
    public function fetchTypeProductionssuppliers($type_id) {

        $table = $this->getTable();
        $select = $table->select()->where('productionssupplier_type_id =' . (int) $type_id);
