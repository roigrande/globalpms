...
{elseif preg_match('/worker\.php/',$smarty.server.SCRIPT_NAME) && (($smarty.request.action eq "new")||($smarty.request.action eq "read")) }
    ...
    <div id="menu-acciones-admin">
        <ul>
            <li>
                <a href="#" class="admin_add" onClick="enviar(this, '_self', 'list', {$smarty.request.page|default:"1"});" value="Cancelar" title="Cancelar">
...
{elseif preg_match('/material\.php/',$smarty.server.SCRIPT_NAME) && (($smarty.request.action eq "new")||($smarty.request.action eq "read")) }
    ...
    {elseif preg_match('/company\.php/',$smarty.server.SCRIPT_NAME) && ($smarty.request.action eq "list") }
    <div id="title-menu"><h2>{$titulo_barra}</h2></div>
    <div id="menu-acciones-admin">
        <ul>
            {acl isAllowed="USER_ADMIN"}
            ...
            <li>
                <a class="admin_add" onclick="enviar(this, '_self', 'new', 0);" onmouseover="return escape('<u>N</u>uevo cliente');" accesskey="N" tabindex="1">
                    <img border="0" src="{$params.IMAGE_DIR}customers_add.png" title="Nuevo" alt="Nuevo"><br />Nueva Compañia
                </a>
            </li>
        </ul>
    </div>
{elseif preg_match('/company\.php/',$smarty.server.SCRIPT_NAME) && (($smarty.request.action eq "new")||($smarty.request.action eq "read")) }
    <div  id="title-menu"><h2>{t}{$smarty.request.action} company{/t}</h2></div>
    <div id="menu-acciones-admin">
        <ul>
            <li>
                <a href="#" class="admin_add" onClick="enviar(this, '_self', 'list', {$smarty.request.page});" value="Cancelar" title="Cancelar">
            ...
            <li>
                ...
                {if isset($company->id) }
                   <a href="#" onClick="javascript:sendFormValidate(this, '_self', 'update', '{$company->id}', 'formulario');">
                       <img border="0" src="{php}echo($this->image_dir);{/php}save.gif" title="Guardar y salir" alt="Guardar y salir"><br /> Guardar y salir
                   </a>
